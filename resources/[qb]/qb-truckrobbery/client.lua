...
CreateThread(function()
	while true do
		...
		if dist <= 25.0 then
			...
			if dist <= 2.0 then
				DrawText3D(Config.MissionMarker.x, Config.MissionMarker.y, Config.MissionMarker.z, '~b~[E]~w~ Missiyanı qəbul et')
...
RegisterNetEvent('AttackTransport:InfoForLspd', function(x, y, z)
	if PlayerJob ~= nil and PlayerJob.name == 'police' then
		if PoliceBlip == 0 then
			...
			AddTextComponentSubstringPlayerName('Pul daşıyan zirehli maşın hücumu')
...
RegisterNetEvent('qb-armoredtruckheist:client:robberyCall', function(streetLabel, coords)
	if PlayerJob.name == 'police' then
		...
		BeginTextCommandSetBlipName('STRING')
		AddTextComponentSubstringPlayerName('10-90: Zirehli maşın qarəti')
		EndTextCommandSetBlipName(blip)
		while transG ~= 0 do
			Wait(180 * 4)
			transG = transG - 1
...
function MissionNotification()
	...
	TriggerServerEvent('qb-phone:server:sendNewMail', {
		sender = 'The Boss',
		subject = 'Yeni hədəf',
		message = 'Pul qazanmaq istəyirsən? Yaxşı... Silahını götür və işə başla... yerini göndərirəm.',
	})
end
	...
	CreateThread(function()
		...
		while not spawned do
			...
			if distance < 200 then
				...
				BeginTextCommandSetBlipName('STRING')
				AddTextComponentSubstringPlayerName('Pul olan furqon')
		...
		while MissionStart == 1 do
			...
			if dist <= 55.0 then
				...
				if warning == 0 then
					warning = 1
					stopAndBeAngry()
					QBCore.Functions.Notify('Bombası yerləşdirmədən əvvəl keşikçilərdən qurtul.', 'error')
					AlertPolice()
				end
	...
	if IsVehicleStopped(transport) then
		if IsVehicleSeatFree(transport, -1) and IsVehicleSeatFree(transport, 0) and IsVehicleSeatFree(transport, 1) and GuardsDead == 1 then
			if not IsEntityInWater(PlayerPedId()) then
				...
				hideLastHint()
				QBCore.Functions.Notify('Yük ' .. Config.TimeToBlow .. ' saniyə sonra partladılacaq.', 'error')
				...
				BlownUp = 1
				lootable = 1
				QBCore.Functions.Notify('Pulu yığmağa başlaya bilərsiniz.', 'success')
				RemoveBlip(TruckBlip)
			else
				QBCore.Functions.Notify('Sudan çıx', 'error')
			end
		else
			QBCore.Functions.Notify('Yükü yerləşdirmək üçün maşın boş olmalıdır', 'error')
		end
	else
		QBCore.Functions.Notify('Hərəkətdə olan maşını qarət edə bilməzsən.', 'error')
	end
end
...
function TakingMoney()
	...
	FreezeEntityPosition(PlayerPedId(), true)
	QBCore.Functions.Notify('Pulu çantaya yığırsan', 'success')
