...
RegisterServerEvent('AttackTransport:akceptujto', function()
	...
	if ActiveMission == 0 then
		if accountMoney < Config.ActivationCost then
			TriggerClientEvent('QBCore:Notify', _source, 'Missiyanı qəbul etmək üçün bankda ' .. Config.Currency .. '' .. Config.ActivationCost .. ' olmalıdır')
		else
			...
			if copsOnDuty >= Config.ActivePolice then
				...
				TriggerClientEvent('QBCore:Notify', _source, 'Missiyanı aktivləşdirmək üçün ən azı ' .. Config.ActivePolice .. ' polis lazımdır.')
			end
		end
	else
		TriggerClientEvent('QBCore:Notify', _source, 'Artıq kimsə bu missiyanı həyata keçirir')
	end
end)
...
RegisterServerEvent('AttackTransport:graczZrobilnapad', function(lootTime)
	...
	TriggerClientEvent('QBCore:Notify', _source, 'Furqondan ' .. bags .. ' pul çantası götürdün')
