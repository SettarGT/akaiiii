...
local function onPlayerConnecting(name, _, deferrals)
    ...
    -- 196 RP | Azerbaijan Role Play — öz whitelist qapımız
    -- (196rp_whitelist resursu DB əsaslı qeydiyyatı təmin edir)
    if GetResourceState('196rp_whitelist') == 'started' and not IsPlayerAceAllowed(src, 'qbcore.god') then
        Wait(0)
        deferrals.update('196 RP | Whitelist yoxlanılır...')
        local canJoin, deniedReason = exports['196rp_whitelist']:CanJoin(src)
        if not canJoin then
            return deferrals.done(deniedReason)
        end
    end
